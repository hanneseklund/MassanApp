# Translate seeded content — 04c-cont-m: Nordbygg exhibitors M
...
- parent: see
  ...
  continuation tasks. This file is the M portion (alone — at 36
  rows it is the largest of the K–R letters and matches the H
  precedent from 04b for a single-letter session).
- shared tooling: see
...
`name` starts with the letter **M** (or `m`). At time of split this
is **36 rows with non-null descriptions**.
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04c-cont-m.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04c-cont-m.json`
...
- Verify with a spot-check select (any updated M row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the M range. Spot-check 5–10 cards;
  both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than M in
  this task — K+L, N+O, P, Q+R each land in their own
