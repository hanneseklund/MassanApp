# Translate seeded content — 04c-cont-k-l: Nordbygg exhibitors K and L
...
- parent: see
  ...
  continuation tasks. K and L each fit in a single session, and
  together come in just below the per-letter precedent (~30 rows),
...
`name` starts with the letter **K** or **L** (case-insensitive).
At time of split this is **30 rows with non-null descriptions**
(18 for K, 12 for L).
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04c-cont-k-l.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04c-cont-k-l.json`
...
- Verify with a spot-check select (any updated K or L row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the K and L ranges. Spot-check 5–10
  cards; both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than K or L
  in this task — M, N+O, P, Q+R each land in their own
