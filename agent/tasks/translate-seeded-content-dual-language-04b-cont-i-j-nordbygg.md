# Translate seeded content — 04b-cont-i-j: Nordbygg exhibitors I and J
...
- parent: see
  ...
  continuation tasks. I and J each have only a small number of
  rows, so they share this single file.
- shared tooling: see
...
`name` starts with the letter **I** or **J** (case-insensitive).
At time of split this is **18 rows with non-null descriptions**
(11 for I out of 17 total; 7 for J out of 13 total).
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04b-cont-i-j.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04b-cont-i-j.json`
...
- Verify with a spot-check select (any updated I or J row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the I and J ranges. Spot-check the
  cards; both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than I or J
  in this task — E, F, G, H each land in their own
