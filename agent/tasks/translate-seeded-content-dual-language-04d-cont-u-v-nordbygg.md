# Translate seeded content — 04d-cont-u-v: Nordbygg exhibitors U and V
...
- parent: see
  ...
  U is tiny (3 rows), so it shares this file with V (27 rows).
- shared tooling: see
...
`name` starts with the letter **U** or **V** (case-insensitive).
At time of split this is **30 rows with non-null descriptions**
(3 for U, 27 for V) where `description->'en' = description->'sv'`.
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04d-cont-u-v.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04d-cont-u-v.json`
...
- Verify with a spot-check select (any updated U or V row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the U and V ranges. Spot-check the
  cards; both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than U or V
  in this task — S, T, W, X–Z each land in their own
