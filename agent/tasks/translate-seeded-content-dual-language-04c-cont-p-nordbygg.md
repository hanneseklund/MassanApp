# Translate seeded content — 04c-cont-p: Nordbygg exhibitors P
...
- parent: see
  ...
  continuation tasks. This file is the P portion (alone — at 31
  rows it is comfortably within the per-letter precedent from 04b).
- shared tooling: see
...
`name` starts with the letter **P** (or `p`). At time of split this
is **31 rows with non-null descriptions**.
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04c-cont-p.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04c-cont-p.json`
...
- Verify with a spot-check select (any updated P row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the P range. Spot-check 5–10 cards;
  both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than P in
  this task — K+L, M, N+O, Q+R each land in their own
