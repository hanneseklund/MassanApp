...
`name` starts with the letters **K, L, M, N, O, P, Q, or R**
(~133 rows). Most carry Swedish-only source copy; produce a natural
English counterpart. A minority carry English-only source copy;
produce a natural Swedish counterpart.
...
for every Nordbygg row in the K–R range:
...
- Reuse (or extend) the per-row translation-payload + applier
  ...
  `scripts/translations/exhibitors-04c.json` so it can be reviewed
...
- Toggle the language flag in the running app:
  - Open Nordbygg 2026 Exhibitors and scroll to the K–R range.
    Spot-check 10–15 cards; both languages render fluently.

## Scope (do not)

- Do **not** translate descriptions for Nordbygg names outside K–R,
