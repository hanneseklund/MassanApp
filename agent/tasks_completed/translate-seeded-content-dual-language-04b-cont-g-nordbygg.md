# Translate seeded content — 04b-cont-g: Nordbygg exhibitors G
...
- parent: see
  ...
  continuation tasks. This file is the G portion.
- shared tooling: see
...
`name` starts with the letter **G** (or `g`). At time of split this
is **24 rows with non-null descriptions** (out of 36 total G-rows;
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04b-cont-g.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04b-cont-g.json`
...
- Verify with a spot-check select (any updated G-row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the G range. Spot-check 5–10 cards;
  both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than G in
  this task — E, F, H, I, J each land in their own
