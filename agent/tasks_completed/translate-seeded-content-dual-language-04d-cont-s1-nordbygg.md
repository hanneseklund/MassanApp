# Translate seeded content — 04d-cont-s1: Nordbygg exhibitors S (first half, Sa–Sm)
...
`name` starts with **S** and falls in the alphabetical first half
of the S range — from `S:t Eriks` through `Smartproduktion Sverige AB`
(inclusive). At time of split this is **35 rows with non-null
descriptions** where `description->'en' = description->'sv'`.

The companion sub-task `-04d-cont-s2-…` covers the second half
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04d-cont-s1.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04d-cont-s1.json`
...
- Toggle the language flag in the running app, open Nordbygg 2026
  ...
  the Sa–Sm sub-range; both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for the second-half S range
  (`SoftOne` onwards) or for letters T–Z in this task — those land
  in their own `-04d-cont-…md` sub-tasks.
