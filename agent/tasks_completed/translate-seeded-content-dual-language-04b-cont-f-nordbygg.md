# Translate seeded content — 04b-cont-f: Nordbygg exhibitors F
...
- parent: see
  ...
  continuation tasks. This file is the F portion.
- shared tooling: see
...
`name` starts with the letter **F** (or `f`). At time of split this
is **25 rows with non-null descriptions** (out of 38 total F-rows;
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04b-cont-f.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04b-cont-f.json`
...
- Verify with a spot-check select (any updated F-row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the F range. Spot-check 5–10 cards;
  both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than F in
  this task — E, G, H, I, J each land in their own
