# Translate seeded content — 04a-cont-d: Nordbygg exhibitors D
...
- parent: see
  ...
  continuation tasks (this file is the D portion).
...
`name` starts with the letter **D** (or `d`). At time of split this
is **24 rows with non-null descriptions** (out of 36 total D-rows;
the other 12 have no description).
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04a-cont-d.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04a-cont-d.json`
...
- Verify with a spot-check select (any updated D-row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the D range. Spot-check 5–10 cards;
  both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than D in this
  task — A is done in 04a, and B/C land in their own
  `-04a-cont-b.md` and `-04a-cont-c.md` sub-tasks.
...
- If the per-row translation list is still too large for one
  session, split D into a smaller batch and create a
  `-04a-cont-d2.md` for whatever remains. Do not ship a
  half-applied seed.
