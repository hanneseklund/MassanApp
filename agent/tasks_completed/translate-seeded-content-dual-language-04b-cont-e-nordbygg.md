# Translate seeded content — 04b-cont-e: Nordbygg exhibitors E
...
- parent: see
  ...
  continuation tasks. This file is the E portion.
- shared tooling: see
...
`name` starts with the letter **E** (or `e`). At time of split this
is **40 rows with non-null descriptions** (out of 54 total E-rows;
the other 14 have no description).
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04b-cont-e.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04b-cont-e.json`
...
- Verify with a spot-check select (any updated E-row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the E range. Spot-check 5–10 cards;
  both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than E in
  this task — F, G, H, I, J each land in their own
...
- If the per-row translation list is still too large for one
  session, split E into a smaller batch and create a
  `-04b-cont-e2.md` for whatever remains. Do not ship a
  half-applied seed.
