# Translate seeded content — 04a-cont-b: Nordbygg exhibitors B
...
- parent: see
  ...
  (in `tasks_completed/`). The 04a parent was scoped to simulated +
  Nordbygg A–D but was too large for one agent session, so it was
  split. The simulated exhibitors and the A-letter Nordbygg
  exhibitors were translated in 04a; B/C/D are split into three
  continuation tasks (this file is the B portion).

This task **depends on** `-04a-…` having landed (the tooling and the
shared terminology comment in `seed.sql` are already in place). It is
**independent** of the other 04a-cont sub-tasks and they can land in
any order.

## Objective

Backfill natural translations for Nordbygg 2026 exhibitors whose
`name` starts with the letter **B** (or `b`). At time of split this
is **35 rows with non-null descriptions** (out of 55 total B-rows;
the other 20 have no description).
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04a-cont-b.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04a-cont-b.json`
...
- Verify with a spot-check select (any updated B-row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the B range. Spot-check 5–10 cards;
  both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than B in this
  task — A is done in 04a, and C/D land in their own
  `-04a-cont-c.md` and `-04a-cont-d.md` sub-tasks.
...
- If the per-row translation list is still too large for one
  session, split B into a smaller batch (e.g. by booth-prefix or
  half the alphabet) and create a `-04a-cont-b2.md` for whatever
  remains. Do not ship a half-applied seed.
