# Translate seeded content — 04c-cont-q-r: Nordbygg exhibitors Q and R
...
- parent: see
  ...
  continuation tasks. Q is tiny (1 row), so it shares this file
  with R (19 rows).
- shared tooling: see
...
`name` starts with the letter **Q** or **R** (case-insensitive).
At time of split this is **20 rows with non-null descriptions**
(1 for Q, 19 for R).
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04c-cont-q-r.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04c-cont-q-r.json`
...
- Verify with a spot-check select (any updated Q or R row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the Q and R ranges. Spot-check the
  cards; both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than Q or R
  in this task — K+L, M, N+O, P each land in their own
