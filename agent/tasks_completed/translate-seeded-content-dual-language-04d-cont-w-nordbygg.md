# Translate seeded content — 04d-cont-w: Nordbygg exhibitors W
...
`name` starts with the letter **W** (case-insensitive). At time of
split this is **20 rows with non-null descriptions** where
...
- Use the same applier-payload pattern as 04a:
  - The applier script `scripts/apply_exhibitor_translations.mjs`
    ...
    a payload at `scripts/translations/exhibitors-04d-cont-w.json`
    of shape `[{ id, en, sv }, ...]`.
...
- Apply the payload locally:
  `node scripts/apply_exhibitor_translations.mjs scripts/translations/exhibitors-04d-cont-w.json`
...
- Verify with a spot-check select (any updated W row).
...
- Toggle the language flag in the running app, open Nordbygg 2026
  Exhibitors and scroll to the W range. Spot-check the cards;
  both languages render fluently.

## Scope (do not)

- Do **not** translate exhibitor `name`.
- Do **not** translate descriptions for letters other than W in
  this task — S, T, U–V, X–Z each land in their own
