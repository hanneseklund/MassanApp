...
preview, slugged from the upstream name). A seeding helper —
the `seed-nordbygg-exhibitors` Supabase Edge Function — fetches the
upstream API and upserts the rows; it can be re-invoked to refresh
the catalog when the upstream data changes. The downloaded JPGs are
not regenerated by the function and must be refreshed alongside any
re-seed if upstream image URLs change.
